...
export class ConfiguratorService {
  ...
  readonly currentPrice = computed(() => {
    ...
    const yokePrice = this.currentWheelIsYoke() ? 1000 : 0;
    const towHitchPrice = this.currentTowHitchIsSelected() ? 1000 : 0;
    if (color && config)
      return color.price + config.price + yokePrice + towHitchPrice;
    else return 0;
  });
